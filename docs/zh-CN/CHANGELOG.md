...
- **i18n**：i18n 在模块加载时自动初始化。`initMiddlewareI18n`
  不再对外导出，调用方 无需再调用。若需设置中间件消息的语言，请使用
  `setMiddlewareLocale`。翻译函数 `$tr` 在首次使用时若尚未初始化会自动初始化。
- **依赖**：`@dreamer/runtime-adapter` 升级至 ^1.0.15，`@dreamer/service` 升级至
  ^1.0.2，`@dreamer/test` 升级至 ^1.0.10。
